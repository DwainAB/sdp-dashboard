    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-white">Clients en attente de validation</h1>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-gray-500">
          ...
          <h3 className="text-sm font-medium text-gray-400">Aucun client en attente</h3>
          <p className="text-xs text-gray-600 mt-1">Tous les clients ont été traités.</p>
        </div>
      ) : (
        <>
                ...
                {reviews.map(review => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <span className="text-[10px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 px-1.5 py-0.5 rounded-full font-medium">
                        {review.type || 'N/A'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-white">{review.first_name || 'N/A'}</td>
                    <td className="px-4 py-3 text-white">{review.last_name || 'N/A'}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className="text-gray-300">{review.email || 'N/A'}</span>
                        {review.verified_email === '1' && (
                          <span className="text-emerald-400 text-xs" title="Email vérifié">✓</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className="text-gray-300">{review.phone || 'N/A'}</span>
                        {review.verified_phone === '1' && (
                          <span className="text-emerald-400 text-xs" title="Téléphone vérifié">✓</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-300">{review.city || 'N/A'}</td>
                    <td className="px-4 py-3 text-gray-300">{review.country || 'N/A'}</td>
                    <td className="px-4 py-3 text-gray-300">{review.formulas?.[0]?.reference || 'N/A'}</td>
                    <td className="px-4 py-3 text-gray-300">{review.job || 'N/A'}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={e => { e.stopPropagation(); handleValidate(review.id) }}
                          className="text-emerald-400 hover:text-emerald-300 bg-emerald-400/10 hover:bg-emerald-400/20 px-2 py-1 rounded text-xs font-medium transition-colors"
                        ...
                        <button
                          ...
                          className="text-red-400 hover:text-red-300 bg-red-400/10 hover:bg-red-400/20 px-2 py-1 rounded text-xs font-medium transition-colors"
            ...
            <div className="flex items-center justify-between text-xs text-gray-400">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="px-2 py-1 rounded bg-gray-800 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  ‹
                </button>
      ...
      {showEditModal && editingCustomer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={handleCancelEdit}>
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  {editingCustomer.verified_email === '1' && (
                    <span className="text-[10px] bg-emerald-400/10 text-emerald-400 border border-emerald-400/30 px-1.5 py-0.5 rounded-full">✓ Email vérifié</span>
                  )}
                  {editingCustomer.verified_email === '0' && (
                    <span className="text-[10px] bg-red-400/10 text-red-400 border border-red-400/30 px-1.5 py-0.5 rounded-full">✗ Email non vérifié</span>
                  )}
                  {editingCustomer.verified_phone === '1' && (
                    <span className="text-[10px] bg-emerald-400/10 text-emerald-400 border border-emerald-400/30 px-1.5 py-0.5 rounded-full">✓ Téléphone vérifié</span>
                  )}
                  {editingCustomer.verified_phone === '0' && (
                    <span className="text-[10px] bg-red-400/10 text-red-400 border border-red-400/30 px-1.5 py-0.5 rounded-full">✗ Téléphone non vérifié</span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleCancelEdit} className="text-gray-400 hover:text-white text-xl">&times;</button>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-6">
                  {!isEditing ? (
                    <>
                      <div>
                        <h4 className="text-sm font-medium text-gray-400 mb-3">Informations personnelles</h4>
                        <div className="space-y-2">
                          {[
                            ...
                            <div key={label as string} className="flex justify-between text-sm">
                              ...
                              <span className="text-white">{value || 'Non renseigné'}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div>
                        <h4 className="text-sm font-medium text-gray-400 mb-3">Localisation</h4>
                        <div className="space-y-2">
                          {[
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {['first_name', 'last_name', 'email', 'phone', 'job'].map(field => (
                            <div key={field}>
                              <label className="text-xs text-gray-500 mb-1 block capitalize">{field.replace('_', ' ')}</label>
                              <input
                                type="text"
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {['city', 'country'].map(field => (
                            <div key={field}>
                              <label className="text-xs text-gray-500 mb-1 block capitalize">{field}</label>
                              <input
                                type="text"
                                value={editForm[field] || ''}
                                onChange={e => handleFormChange(field, e.target.value)}
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-3">Document scanné</h4>
                  {filesLoading ? (
                    <div className="flex items-center justify-center h-48 text-gray-500 text-sm">Chargement...</div>
                  ) : customerFiles.length > 0 ? (
                    <div>
                      <div className="bg-gray-950 border border-gray-800 rounded-lg overflow-hidden mb-2">
                        <img
              ...
              {!isEditing && formulas.length > 0 && (
                <div className="mt-6 pt-6 border-t border-gray-800">
                  <h4 className="text-sm font-medium text-gray-400 mb-3">Formule</h4>
                  {formulas.map(formula => (
                    <div key={formula.id} className="bg-gray-950 border border-gray-800 rounded-lg p-4 mb-3">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="text-xs">
                          <span className="text-gray-500">Référence :</span>
                          <span className="text-white ml-1">{formula.reference || 'Non renseigné'}</span>
                        </div>
                        {formula.perfume_name && (
                          <div className="text-xs">
                            <span className="text-gray-500">Parfum :</span>
                            <span className="text-white ml-1">{formula.perfume_name}</span>
                          </div>
                        )}
                      </div>
                    ...
                    <div key={formula.id} className="bg-gray-950 border border-gray-800 rounded-lg p-4 mb-3">
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          className="w-full bg-gray-900 border border-gray-800 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-indigo-500 transition-colors"
                        />
                      </div>
                      <div className="grid grid-cols-3 gap-4">
                        {(['top_notes', 'heart_notes', 'base_notes'] as const).map((noteType, ti) => (
                              ...
                              {formula[noteType].map((note, noteIndex) => (
                                <div key={note.id || noteIndex} className="flex gap-1 items-center">
                                  <select
                                    ...
                                    className="flex-1 bg-gray-900 border border-gray-800 rounded px-2 py-1 text-xs text-white outline-none focus:border-indigo-500"
                                  >
                                  ...
                                  <input
                                    ...
                                    className="w-16 bg-gray-900 border border-gray-800 rounded px-2 py-1 text-xs text-white outline-none focus:border-indigo-500"
                                    placeholder="Qté"
                                  />
                                  <button
                                    onClick={() => {
              ...
              {isEditing && (
                <>
                  {modalError && (
                    <div className="bg-red-900/20 border border-red-800 text-red-400 text-sm px-4 py-2 rounded-lg mt-6">⚠️ {modalError}</div>
                  )}
      ...
      {showFormulaModal && selectedFormula && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowFormulaModal(false)}>
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
              <h3 className="text-white font-bold">Formule {selectedFormula.id}</h3>
              <button onClick={() => setShowFormulaModal(false)} className="text-gray-400 hover:text-white text-xl">&times;</button>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-3">Document associé</h4>
                  {selectedFormula.file_id && customerFiles.find(f => f.id === selectedFormula.file_id) ? (
                    <img
                      ...
                      className="w-full rounded-lg cursor-pointer border border-gray-800"
                      onClick={() => {
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-3">Composition de la formule</h4>

        ...
        <div className="h-14 border-b border-gray-800 flex items-center px-6 gap-4">
          ...
          <div className="relative ml-auto">
            <button
              ...
              className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-xs text-gray-300 px-3 py-1.5 rounded-lg transition-colors"
            >
              {currentProject && (
                ...
                <div className="absolute right-0 top-full mt-1 w-52 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-20 py-1">
                  {projects.map((p) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-left transition-colors ${p.slug === normalizedSlug ? 'text-indigo-400 bg-indigo-600/10' : 'text-gray-300 hover:bg-gray-700'}`}
                    >
                      <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: p.color }} />
                      {p.name}
                      {p.status === 'maintenance' && (
                        <span className="ml-auto text-[10px] text-amber-400">Maint.</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-bold text-white">{activeSectionData?.name ?? dashboard.name}</h2>
                ...
                {filteredMetrics.length === 0 && filteredCharts.length === 0 && (
                  <p className="text-gray-500 text-sm text-center py-12">Aucune donnée pour cette section</p>
                )}
              </div>
            )
          ) : null}
